...
def convert_Page_ChunkinChar(pdf_texts, chunk_size = 1000, chunk_overlap = 100):
    ...
    print(f"\nTotal number of chunks (document splited by max char = {chunk_size}): \
...
def convert_Chunk_Token(text_chunksinChar, sentence_transformers_model, chunk_overlap=100, tokens_per_chunk=128):
    ...
    print(f"\nTotal number of chunks (document splited by {tokens_per_chunk} tokens per chunk): {len(text_chunksinTokens)}")
...
def load_multiple_pdfs_to_ChromaDB(collection_name,sentence_transformer_model, chromaDB_path, file_path):
  category= "Banka"
...
if __name__ == "__main__":
    chroma_client, chroma_collection= load_multiple_pdfs_to_ChromaDB("Akbank", sentence_transformer_model, chromaDB_path, file_path)